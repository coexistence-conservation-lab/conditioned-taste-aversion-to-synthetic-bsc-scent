...
days_of_week <- weekdays(dates)

# Combine with original dataset
df <- data.frame(Date = dates, DayOfWeek = days_of_week)

# Print the resulting dataframe
print(df)

# Remove Fridays from the dataframe
df_filtered <- subset(df, DayOfWeek != "Friday")
...
data <- data %>%
  ...
  mutate(fox_visit_pa = ifelse(fox_visit_1_pa !=0, "Yes", 
                        ifelse(fox_visit_2_pa !=0, "Yes",
                        ifelse(fox_visit_3_pa !=0, "Yes", 
                        ifelse(fox_visit_4_pa !=0, "Yes", 
                        ifelse(fox_visit_5_pa !=0, "Yes", "No")))))) %>%
  ...
  mutate(raven_visit_pa = ifelse(raven_visit_1_pa !=0, "Yes", 
                          ifelse(raven_visit_2_pa !=0, "Yes",
                          ifelse(raven_visit_3_pa !=0, "Yes", "No")))) %>%
...
# Model for fox visits by treatment and phase
summary(glm(fox_visit_pa ~ treatment + phase, data=data))
```
...
# Model for bait take by treatment and site
...
summary(glm(fox_visit_n ~ treatment + phase + 
...
summary(glm(raven_visit_n ~ treatment + phase + 
              rainfall_mm + maximum_temperature_c, data=data))
```
...
Here we map our responses of `bait_take`, `fox_visit_n`, and `raven_visit_n` across Ginninderry Conservation Corridor.
...
```{r, eval=FALSE, include=FALSE}
# Save the plot as a jpeg
ggsave(plot=mod_map, "figures/bait take map.jpeg", 
...
mod_map <- ggmap(map) + 
  geom_point(data=data, shape=21, size=2, stroke=1, col="white", alpha=0.2,
             aes(x=long, y=lat, fill=fox_visit_n)) + 
  theme(axis.text.x=element_blank(), 
        axis.text.y=element_blank(), 
        axis.ticks.x=element_blank(), 
        axis.ticks.y=element_blank(), 
...
```{r, eval=FALSE, include=FALSE}
# Save the plot as a jpeg
ggsave(plot=mod_map, "figures/fox visits map.jpeg", 
       height=4, width=6, dpi=300)
```
...
mod_map <- ggmap(map) + 
  geom_point(data=data, shape=21, size=2, stroke=1, col="white", alpha=0.2,
             aes(x=long, y=lat, fill=raven_visit_n)) + 
  theme(axis.text.x=element_blank(), 
        axis.text.y=element_blank(), 
        axis.ticks.x=element_blank(), 
        axis.ticks.y=element_blank(), 
...
```{r, eval=FALSE, include=FALSE}
# Save the plot as a jpeg
ggsave(plot=mod_map, "figures/raven visits map.jpeg", 
       height=4, width=6, dpi=300)
```
...
Here we model the relationship between our response variables (i.e., `bait_take`, `fox_vists`, and `raven_visit_n`) and the distance between the bait station and nearby spatial features (i.e., boundary [`dist_bou`], roads [`dist_roads`], construction site [`dist_con`], vegetation strip [`dist_veg`], pondage [`dist_pond`], watercourses [`dist_wcl`]). 

```{r}
# Subset to only variables in the subsequent models
data_mod <- data %>%
  select(bait_take, treatment, fox_visit_n, raven_visit_n, 
...
# Subset to only variables in the subsequent models
data_mod <- data %>%
  select(bait_take, treatment, fox_visit_pa, raven_visit_pa, 
         dist_bou, dist_roads, dist_con, dist_veg, dist_pond, dist_wcl) %>%
  mutate(fox_visit_pa=ifelse(fox_visit_pa=="Yes", 1, 0), 
         raven_visit_pa=ifelse(fox_visit_pa=="Yes", 1, 0)) %>%
  na.omit()

# Fit the model with glm
mod <- glm(fox_visit_pa ~ treatment + dist_bou + dist_roads + 
             dist_con + dist_veg + dist_pond + dist_wcl, 
            data = data_mod, family=binomial, na.action = "na.fail")
...
# Since dist_pond and dist_wcl was non-significant, remove from model
mod <- glm(fox_visit_pa ~ treatment + dist_bou + 
             dist_roads + dist_con + dist_veg, 
            data = data_mod, family=binomial, na.action = "na.fail")
...
```{r}
# Fit the model with glm
mod <- glm(raven_visit_pa ~ treatment + dist_bou + dist_roads + dist_con + 
              dist_veg + dist_pond + dist_wcl, 
            data = data_mod, family=binomial, na.action = "na.fail")
table(data_mod$raven_visit_pa)
# Display summary statistics
summary(mod)

# Since dist_pond and dist_wcl was non-significant, remove from model
mod <- glm(raven_visit_pa ~ treatment + dist_bou + dist_roads + dist_con + dist_veg, 
            data = data_mod, family=binomial, na.action = "na.fail")
...
gcc_map <- ggplot() + 
  ...
  geom_text(data=stations_spdf, aes(x=long, y=lat, label=name), 
            size=3, color="black") +
